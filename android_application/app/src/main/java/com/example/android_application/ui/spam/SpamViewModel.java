...
public class SpamViewModel extends MailListViewModel {

    private LiveData<List<Mail>> spamMails;
    private final MailRepository mailRepository;

    public SpamViewModel(@NonNull Application application, String currentUserEmail) {
        super(application);
        mailRepository = new MailRepository(application.getApplicationContext());
        getMails("Spam", currentPage);
        spamMails = mailRepository.getSpamMailsLive(currentUserEmail);
    }
    ...
    public static class Factory implements ViewModelProvider.Factory {
        ...
        public <T extends ViewModel> T create(@NonNull Class<T> modelClass) {
            if (modelClass.isAssignableFrom(SpamViewModel.class)) {
                return (T) new SpamViewModel(application, currentUserEmail);
            }
