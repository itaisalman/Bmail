...
function checkPostArguments({ receiver, draft }) {
  ...
  if (checkIfValid(receiver) && checkIfExist(receiver)) {
    if (draft_string.toLowerCase() === "true") return null;
  }
  return {
    statusCode: 400,
    error: "Invalid/Missing Receiver",
  };
  // // Check if valid draft mail
  // if (draft_string.toLowerCase() === "true")
  //   if (receiver || title || content) return null;

  // if (draft_string.toLowerCase() === "false")
  //   if (checkIfValid(receiver)) if (checkIfExist(receiver)) return null;
...
exports.addMail = async ({ headers, body }, res) => {
  ...
  console.log("Extracted title:", extracted_title);
  console.log("Extracted content:", extracted_content);
  let command = "GET ";
  try {
